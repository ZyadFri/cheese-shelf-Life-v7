...
export default async function ModelingPage() {
  const { models: modelResponse, manifest } = await api.models();
  const models = [...modelResponse].sort((a, b) => a.validation_rmse - b.validation_rmse);
  ...
  return (
    <PageBody className="relative isolate max-w-[1480px] overflow-hidden pb-12 pt-0 sm:px-5 lg:px-7">
        ...
        <div className="relative min-h-[170px] overflow-hidden px-4 py-7 sm:px-7 lg:min-h-[182px] lg:px-9">
          ...
          <div className="relative z-10 max-w-[520px]">
            ...
            <p className="mt-3 max-w-[48ch] text-[0.72rem] leading-5 text-[#74666c] sm:text-[0.78rem]">
              Training and validation results for {models.length} independently-trained model{models.length === 1 ? "" : "s"}, read from saved artifacts. Retraining is CLI-only.
            </p>
            <p className="mt-2 flex items-center gap-1.5 text-[0.54rem] text-[#9a878f]">
              <CalendarClock className="size-3" strokeWidth={1.7} />
              Last updated {formatBuildTime(manifest.created_at_utc)}
            </p>
          </div>
        </div>
      </section>
      ...
      <Reveal>
        <section className="relative z-10 -mt-1 grid gap-2.5 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard
            label="Best model"
          ...
          <MetricCard
            ...
            sub={`${testPct}% of dataset, held out`}
        ...
        <Reveal>
          <article className="overflow-hidden rounded-[20px] border border-[#eadfe3] bg-white/94 shadow-[0_18px_48px_-40px_rgba(76,27,44,.42)]">
            <div className="px-4 pb-2 pt-3.5 sm:px-5">
              ...
              <p className="mt-0.5 text-[0.56rem] text-[#95838a]">Ranked by validation RMSE (lower is better).</p>
            </div>
        ...
        <section className="mt-3 overflow-hidden rounded-[20px] border border-[#eadfe3] bg-white/94 shadow-[0_18px_48px_-40px_rgba(76,27,44,.42)]">
          <div className="flex items-center gap-2 border-b border-[#eee5e8] px-4 py-3 sm:px-5">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[0.5rem] text-[#96838b]">Retraining happens only via <code className="font-mono">python train_models.py</code>, never from this dashboard.</p>
            </div>
          </div>
          <div className="grid divide-y divide-[#eee5e8] md:grid-cols-3 md:divide-x md:divide-y-0 xl:grid-cols-6">
            ...
            <RunDetail icon={<BookOpen className="size-3.5" />} label="Total rows (real / synthetic)" value={`${manifest.n_total.toLocaleString()} (${manifest.n_real_paper_derived.toLocaleString()} / ${manifest.n_synthetic.toLocaleString()})`} />
            ...
            <RunDetail icon={<Database className="size-3.5" />} label="Dataset" value={compactPath(manifest.dataset_path)} />
